 ...
 * 而在具体使用的时候就跟调用本地方法一点的进行调用即可。 由于Feign 是基于 Ribbon 实现的，所以它自带了客户端负载均衡功能，也可以通过 Ribbon 的 IRule 进行策略扩展。
 ...
 * 只有后两者的话，启动的时候其实是不会有任何异常被抛出的， 但是如果细心地查看了启动 log 的话，其中有这么一条可以看出实际上确实是没有获取到任何服务的:
